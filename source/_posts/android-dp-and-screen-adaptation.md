...
> 通过将屏幕尺寸描述为密度无关像素(dp)的度量值，Android 允许您创建专为非常具体的屏幕尺寸而设计的布局，同时让您不必对不同的像素密度有任何担心。

该最小宽度的计算方式为：`屏幕宽度(px)/屏幕密度缩放倍数`。如你的屏幕为720x1280 xhdpi，那么720/2=360。

使用如下：`res/values-sw360dp/dimens.xml`，就是满足最小宽度(dp)为360dpi的屏幕进行适配，这种方式是分辨率+屏幕密度(dpi)，比较适合。

#### **使用代码强行设置Activity的`屏幕密度`(dpi)**
...
获取设备`屏幕密度`(dpi)：`adb shell wm density`
设置`屏幕密度`(dpi) ：`adb shell wm density 320`
重置`屏幕密度`(dpi) ：`adb shell wm density reset`
